...
def train_BACK(model, x, y, n_epochs=10, lr=1e-3, batch_size=32, tol=1e-4):
    ...
    for epoch in range(n_epochs):
        ...
        # Average epoch loss
        epoch_loss /= len(batch_X) # X_train
        train_error /= len(batch_X) # X_train
...
if __name__ == "__main__":
    ...
    te_dfa, loss_dfa, angles = train_DFA(model_DFA, X_train, y_train, n_epochs=epochs, lr=learning_rate, batch_size=batch_size, tol=tol)
